...
import {CommandContext, EventContext, HandlerStatus} from "@atomist/skill/lib/handler";
...
export function buildMessage(
  ...
  if (
    ...
    header = `*${params.project.id.owner}/${params.project.id.repo}* at ${url(
  ...
  const finalState = (step === steps || state === SkillStepState.Failure) ? state : SkillStepState.InProcess;
    ...
    attachments: [
      {
        ...
        thumb_url:
          `https://badge.atomist.com/v2/progress/${finalState}/${step}/${steps}?image=no&counter=no&color=${color}`,
...
export async function retrieveGcpCreds(
  ...
  const result: GcpAuthCred = await ctx.graphql.query(query, {
    id: ctx.configuration[0].resourceProviders.gcp.selectedResourceProviders[0].id,
  });
...
const convertTfVarArray = (data: string) => {
    ...
    if (splitData.length !== 2) {
        throw new Error(`Could not process supplied Terraform var: ${data}.  Is it delimited with "="?`);
    }
...
export async function setParams(ctx: EventContext, params: TerraformRegistration): Promise<TerraformRegistration> {
  ...
  params.vars = ctx.configuration[0].parameters.cmdlinevars ?
    (ctx.configuration[0].parameters.cmdlinevars as string[]).map(convertTfVarArray) : [];
  ...
  params.autoApprove = ctx.configuration[0].parameters.autoApprove ?
    ctx.configuration[0].parameters.autoApprove : false;
  params.version = ctx.configuration[0].parameters.version ?
    ctx.configuration[0].parameters.version : undefined;
...
export async function hclCodePresent(
  ...
  return results.length > 0
    ? { code: 0 }
    : {code: 1, reason: `No HCL code exists in supplied base location [${params.baseLocation}]`};
}

export async function findCommitterScreenName(ctx: EventContext | CommandContext, sha: string): Promise<string> {
