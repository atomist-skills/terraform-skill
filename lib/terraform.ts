...
export async function shouldRunTfInit(
	context: unknown,
	reg: TerraformRegistration,
): Promise<boolean> {
	// Init will be false by default.  If it is false, the user did NOT disable init and we should run it.
	return !reg.init;
}
export async function shouldRunSelectTfWorkspace(
	context: unknown,
	reg: TerraformRegistration,
): Promise<boolean> {
	return !!reg.workspace;
}
export async function shouldRunTerraformApply(
	context: unknown,
	reg: TerraformRegistration,
): Promise<boolean> {
	return !!reg.autoApprove || (context as any).tfApply === true;
}
export async function shouldRunTfVersion(
	context: unknown,
	reg: TerraformRegistration,
): Promise<boolean> {
	return !!reg.version;
...
export const terraformBackendGrammar = Microgrammar.fromString(
...
export const ValidateTerraform: Step<
...
export const SetTerraformVersion: Step<
	EventContext<OnPushSubscription>,
	TerraformRegistration
> = {
...
export const InitTerraform: Step<
	EventContext<OnPushSubscription>,
	TerraformRegistration
> = {
...
export const SelectWorkspaceTerraform: Step<
	EventContext<OnPushSubscription>,
	TerraformRegistration
> = {
...
export const RunTerraformPlan: Step<
	EventContext<OnPushSubscription>,
	TerraformRegistration
> = {
	name: "Terraform Plan",
	run: async (ctx, params) => {
		...
		// Execute Plan
		const result = await executeTfAction("plan", newArgs);
		...
		msg.attachments[msg.attachments.length - 1].ts = Math.floor(
			Date.now() / 1000,
		);
		...
		if (ctx.data.Push[0].repo.channels.length > 0) {
			...
			notify = {
				type: "person",
				value: await findCommitterScreenName(
					ctx,
					ctx.data.Push[0].after.sha,
				),
			};
		}
...
export const RunTerraformApply: Step<
	EventContext<OnPushSubscription>,
	TerraformRegistration
> = {
...
export async function selectTfWorkspace(
	...
	const result = await registration.project.spawn(
		...
		{
			cwd: registration.baseLocation
				? `${registration.project.path()}/${registration.baseLocation}`
...
export async function runTfValidate(
	registration: TerraformRegistration,
	logger: Logger,
): Promise<HandlerStatus> {
	const result = await registration.project.spawn("terraform", ["validate"], {
		cwd: registration.baseLocation
			? `${registration.project.path()}/${registration.baseLocation}`
			: registration.project.path(),
		env: { ...process.env, ...registration.envVars },
...
export async function runTfInit(
	registration: TerraformRegistration,
	logger: Logger,
): Promise<HandlerStatus> {
...
export async function buildTfVars(
	registration: TerraformRegistration,
): Promise<string[]> {
	const args: string[] = [];
	if (registration.vars) {
		args.push(
			..._.flatten(
				registration.vars.map(a => [`-var`, `${a.tfVar}=${a.value}`]),
			),
		);
	}
...
export async function setTfVersion(
	registration: TerraformRegistration,
): Promise<HandlerStatus> {
	let response: HandlerStatus;
	const result = await registration.project.spawn("tfenv", [
		"use",
		registration.version,
	]);
	if (result.status !== 0) {
		const install = await registration.project.spawn("tfenv", [
			"install",
			registration.version,
		]);
		if (install.status !== 0) {
