...
describe("terraform", () => {
	...
	describe("handler", () => {
		it("should not run if the branch is incorrect", async () => {
			...
			const reason = `Incoming branch [${context.data.Push[0].branch}] does not match configured branch [master], skipping.`;
			assert.strictEqual(resultGood.code, 0, "Result code is not 0!");
			assert.strictEqual(
				resultGood.reason,
				reason,
				"Reason string doesn't match expected!",
			);
		...
		it.skip("should run if the branch is correct", async () => {
			...
			assert.strictEqual(
				...
				"Success",
				"Result is not correct! Should be 'Success'!",
			);
	...
	describe("SetParamsStep", () => {
		it("should set params successfully", async () => {
			const push = JSON.parse(
				fs.readFileSync("test/data/push.json").toString(),
			);
			const context = createFakeContext(push, gcp);
			const params = {};
			await SetParamsStep.run(context as any, params as any);
		...
		it("should set params successfully with default config", async () => {
			const push = JSON.parse(
				fs.readFileSync("test/data/pushNoArgs.json").toString(),
			);
			const context = createFakeContext(push, gcp);
			const params = {};
			await SetParamsStep.run(context as any, params as any);
	...
	describe("initTerraform", () => {
		...
		it("should run if init param is false", async () => {
			...
			assert.strictEqual(
				fake.firstCall.lastArg.cwd,
				"/fake/project/path/tf",
			);
	...
	describe("RunTerraformApply", () => {
		...
		it("should run apply", async () => {
			const push = JSON.parse(
				fs.readFileSync("test/data/pushAutoApprove.json").toString(),
			);
			const context = createFakeContext(push, gcp);
			const fake = sinon.fake.returns({ status: 0 });
			await steps.runSteps({
